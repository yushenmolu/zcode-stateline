...
│ ┌──────┐ ┌────────┐ ┌──────────┐ ┌─────────┐ ┌────────┐ ┌──────────┐ │
│ │◷ 31.2s│ │▸in 7.7M│ │◂out 51.3k│ │◐ 44.3%  │ │↻ 3.4M  │ │⚡32.4tok/s│ │   ← 第二行：彩色指标块
│ └──────┘ └────────┘ └──────────┘ └─────────┘ └────────┘ └──────────┘ │
└────────────────────────────────────────────────────────────┘
...
- **智能贴边状态条（主显示）**：无边框置顶深色小条，自动吸附在 ZCode 主窗口底部外沿并跟随窗口移动；ZCode 最小化或切到其他程序时自动隐藏，回到 ZCode 才显示，从不抢焦点。
  - **两行彩色指标块**：第一行 = 当前模型名（蓝）+ 会话标题（灰）+ 关闭按钮；第二行 = 一排圆角指标块（耗时 / in / out / 缓存命中 / cache read / 速度 tok/s / reasoning），等宽字体防跳字，暗色主题 AA 对比度。
  - **速度块（⚡ tok/s，橙黄）**：最近一次请求的输出速度 = 输出 token ÷ 该次耗时，悬停气泡顺带显示会话平均速度；无数据（如该次耗时缺失）时整块不显示。
  - **靠边收起**：双击状态条任意区域或右键 →「收起到边缘」，收成屏幕底部的迷你把手（`◐ 84.9%` 浓缩缓存命中率）；把手**悬停约 0.5 秒或单击**即展开回完整状态条（重新贴边）。收起状态持久化到配置（`collapsed` 字段），重启后保持；把手同样只随 ZCode 前台显示、可拖动且不出屏。
...
- 命令行验证数据读取（不弹窗）：`python scripts/docked_statusbar.py --once` 输出统计 JSON（含 `speedTokPerSec` 最近一次输出速度与 `speedAvgTokPerSec` 会话平均，无数据为 `null`）。
